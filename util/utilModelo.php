...
//    die();
        ...
        function aumentarSaldo($codigoUsuario, $valor)
        {
            ...
            $valor1 = array("$codigoUsuario");
            $tabla = "usuario";
            $result = $this->mostrarregistros($tabla, $nombreCampo, $valor1);
            $fila = mysqli_fetch_array($result);
            $nombreUsuario = $fila['nombre'];
            echo '<div style=" color: red"> nombre: ' . $nombreUsuario . '</div>';
        ...
        function consultarComisionRango($codigoCabeza,$fechaUltimoPago)
        {
            global $link;
            $consulta = "SELECT COUNT(DISTINCT(codigoHijo)) FROM `registroComision` WHERE codigoCabeza = '$codigoCabeza' AND nivel <> 1 AND (fecha>= '$fechaUltimoPago')";
            echo $consulta . '<br>';
            $query = mysqli_query($link, $consulta);
            //echo "$consulta";
            return $query;
        ...
        function consutarComisionesMes($codigoHijo, $codigoCabeza, $fechaUltimoPago, $padreDeIniciador)
        {
            global $link;
            $consulta = "SELECT COUNT(codigoHijo) FROM `registroComision` WHERE codigoHijo = '$codigoHijo' AND codigoCabeza = '$codigoCabeza' AND (fecha>= '$fechaUltimoPago') AND padreDeIniciador='$padreDeIniciador'";
//            echo $consulta . '<br>';
            $query = mysqli_query($link, $consulta);
            //echo "$consulta";
            return $query;
